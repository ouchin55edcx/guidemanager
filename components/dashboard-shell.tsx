...
export default function DashboardShell({ children, userName }: { children: React.ReactNode; userName?: string }) {
  ...
  return <div className="min-h-screen bg-background text-foreground">
    <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-border bg-background/90 px-4 backdrop-blur md:px-8">
      ...
      <div className="flex items-center gap-3"><label className="hidden items-center gap-2 text-sm text-muted-foreground lg:flex"><CalendarDays className="size-4" />Travel date</label><input type="date" value={date} onChange={e => setDate(e.target.value)} aria-label="Travel date" className="h-9 rounded-xl border border-input bg-card px-2.5 text-sm" /><div className="hidden items-center gap-2 text-sm md:flex"><CircleHelp className="size-4 text-muted-foreground" />Help</div><div className="flex size-9 items-center justify-center rounded-full bg-muted text-xs font-semibold">{(userName || 'O').slice(0, 1).toUpperCase()}</div></div>
    </header>
